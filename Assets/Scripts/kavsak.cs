...
public class kavsak : MonoBehaviour //4. Eğitim
{
    public GameObject basarili;
    public GameObject basarisiz;
    public GameObject textBox;
    public GameObject Car;
    public GameObject trigger4, trigger5, trigger6; //Success triggers
    public GameObject triggerF6, triggerF7, triggerF8, triggerF9; //Fail trigger, F6 ve F7 = Sol, F8 ve F9 = sağ

    public CarController cc;

    public Vector3 PosIn = new Vector3(-16, 0, -100f);
    public Quaternion RotIn = new Quaternion(0, 90, 0, -60);
    ...
    {
        if (currentLine == 2 || currentLine == 4 || currentLine == 6 || currentLine == 12 || currentLine == 16 || currentLine == 18)
        {
            Car.gameObject.GetComponent<CarController>().enabled = true;
        }


        else if (currentLine == 10 || currentLine == 14)
        {
            StartCoroutine(waitingSignal());
            Car.gameObject.GetComponent<CarController>().enabled = true;
        }
        ...
        if (training == 0 && currentLine >= 13)
        {
            currentLine = 12;
        }
        else if (training == 1 && currentLine >= 18)
            currentLine = 18;
        voice.CallVoice();
